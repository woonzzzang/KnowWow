...
export default function TeamPage() {
  ...
  useEffect(() => {
    getPatterns().then(setPatterns).catch((caught) => setError(caught instanceof Error ? caught.message : "처리 경향을 불러오지 못했습니다."));
  }, []);
  ...
  return (
    <>
      ...
      {error && <Card className="mb-5 border-amber-200 bg-amber-50 p-5 text-sm text-amber-900" role="alert">팀의 처리 경향을 불러오지 못했습니다. {error}</Card>}
